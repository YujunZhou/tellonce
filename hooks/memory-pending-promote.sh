...
if [ -n "${_OBS_LOG_FOR_SC}" ] && [ -f "${_OBS_LOG_FOR_SC}" ] && [ -n "${_CUR_SID_SC}" ]; then
  ...
  if [ "${_AGE_SC}" -lt 60 ]; then
    ...
    if [ "${_LAST_SID_SC}" = "${_CUR_SID_SC}" ]; then
      _LAST_DETECTED_SC=$(echo "${_LAST_LINE_SC}" | jq -r '.detection.detected | tostring' 2>/dev/null)
...
printf '%s' "${_INPUT_SC}" | python3 "${_PT_LIB}/pending_queue_manager.py" promote >/dev/null 2>&1 || true
exit 0
