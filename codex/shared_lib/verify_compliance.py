...
Legacy pending-finalize observations are retained for metrics only. Unified
SQLite memory upsert is the sole writer, so this hook never asks an agent to
edit memory projections or observation history.

Scope: cite-rate, lang-ratio, fp-match, and legacy pending counts are log-only.
B4 thresholds remain available for audit metrics via the B4_* env vars below.
...
# B4 thresholds (tunable via env vars)
...
def write_pending_alert(session_id, pending_count, session_age_min, oldest_pending_age_min, retry_count=0):
    ...
    describing legacy pending metrics without changing memory or Stop behavior."""
    ...
    body = f"""# B4 legacy pending audit

**Session**: `{sid_safe}`
**Trigger time**: {_now_utc().isoformat()}
**Historical retry count**: {retry_count}
...
- These legacy observations are metrics only and do not block Stop

## Current writer protocol

Do not edit Markdown projections or historical observation rows. New durable
signals must enter through `memory_upsert.py enqueue`, which owns semantic
resolution, the SQLite transaction, and projection rebuilding.

---
Generated by `verify_compliance.py:write_pending_alert`. Tune via the B4_* env vars documented in verify_compliance.py.
"""
    try:
...
def main():
    ...
    # Avoid duplicate telemetry inside a continuation spawned by another hook.
    ...
    # B4 legacy pending metrics (audit-only).
