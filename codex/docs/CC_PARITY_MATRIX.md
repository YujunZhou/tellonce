...
1. PostToolUse takes the place of Stop hook. CC fires Stop after every text
   ...
   wrapper path (`codex_preftrack exec --`) since codex lacks a per-text
   hook.
2. Retrieve / pending / shadow-alert UPS hooks reuse CC's `lib/*.py`
   ...
   `~/.codex/skills/preference-tracker/`.

## Known CC Bugs / Portability Holes (historical)

- ~~Installs into `~/.claude/skills/preference-tracker`, not Codex paths.~~ Codex
  install now lays out `~/.codex/skills/preference-tracker/` with `codex_preftrack/`
