...
if 'Tellonce install — version ' + version not in (root / 'install.sh').read_text():
...
for name, declaration in [('bootstrap.sh', 'REF="v' + version + '"'),
    ...
    if declaration not in script.read_text():
...
for relative in ('README.md', 'README.zh.md', 'copilot/README.md', 'copilot/README.zh.md',
    ...
    refs = re.findall(r'tellonce/(v[0-9]+[.][0-9]+[.][0-9]+)/', (root / relative).read_text())
