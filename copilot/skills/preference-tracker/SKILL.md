...
> **路径占位说明**: 下文出现的 `<skill_dir>` 默认是 `<skill_dir>/`，`<project_root>` 是当前项目根，`<state_dir>` 是 `<project_root>/.copilot/preference-tracker-state/`。所有路径在运行时由 `lib/path_config.py` 解析（env > `~/.preference-tracker.config.json` > 自动 detect 三层兜底），SKILL.md 不写绝对路径以免污染 Claude 输出。

### Deterministic fingerprint retrieval (UserPromptSubmit hook)

每次用户提交 message, `<plugin_root>/hooks/memory-retrieve-inject.sh` 会:
...
| UserPromptSubmit hook | `<plugin_root>/hooks/memory-retrieve-inject.sh` |
| Stop hook | `<plugin_root>/hooks/memory-verify-compliance.sh` |
...
存储位置 (path_config-driven): `<project_root>/.copilot/preference-tracker/memory/`，其中 `<cwd_escaped>` 是当前项目 cwd 把 `/` 换成 `-`. 真实路径跑 `python3 <skill_dir>/lib/path_config.py` 看 `memory_dir` 字段，或读 `<skill_dir>/lib/path_config.py:get_memory_dir()`.
