...
The Copilot variant **does not have** a per-prompt `memory-retrieve-inject.sh` hook (platform limitation, see the injection-timing note above). Rule injection happens **at session start**: `<plugin_root>/hooks/session-start-inject.sh` will:
1. Read `<skill_dir>/lib/fingerprints.yaml` (**shipped empty**; put your own rules in the gitignored `fingerprints.user.yaml` overlay, and the two are merged at load time) + memory rules
2. Inject the critical/high rules as `additionalContext` at the start of this session's context
...
Each rule injected at session start carries `applies_when: ...` and `condition: ...` read from the memory .md frontmatter. I judge:
...
| Stop hook chain (executed in order) | `check-observation-log.sh` → `memory-deterministic-block.sh` → `memory-verify-compliance.sh` → `memory-shadow-judge.sh` → `memory-pending-promote.sh` (all in `<plugin_root>/hooks/`) |
...
Storage location (path_config-driven): defaults to `<project_root>/.copilot/tellonce/memory/` (project-local). Migration fallback: if that directory has no .md files but the old Claude Code path `~/.claude/projects/<cwd_escaped>/memory` has content, keep using the old path (`<cwd_escaped>` = cwd with `/` replaced by `-`; see `lib/pt_platform.py:default_memory_dir`). For the real path, run `python3 <skill_dir>/lib/path_config.py` and look at the `memory_dir` field, or read `<skill_dir>/lib/path_config.py:get_memory_dir()`.
