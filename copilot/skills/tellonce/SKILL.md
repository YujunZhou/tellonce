...
- 自动检索不注入待澄清记录；用户明确要求检查记忆时可查看，过期项可用 `python <plugin>/lib/memory_upsert.py dismiss --turn-key <id>` 手动移除。
...
| Stop hook chain (executed in order) | `check-observation-log.sh` → `memory-deterministic-block.sh` → `memory-verify-compliance.sh` → `memory-shadow-judge.sh` (all in `<plugin_root>/hooks/`) |
