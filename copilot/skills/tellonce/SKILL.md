...
- enqueue 只写 inbox 并启动 detached worker，立即返回。LLM 语义判断（NOOP/UPDATE/SUPERSEDE/SPLIT/NEW）、合并、事务和投影全部在后台运行；失败按退避重试，达到上限后标记 failed 并停止，不能阻塞当前回复。
...
^\*\*Decision\*\*: (NOOP|UPDATE|SUPERSEDE|SPLIT|NEW)\b.*— because .+$
```
