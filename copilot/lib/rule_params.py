#!/usr/bin/env python3
"""Phase 7 simple adaptive thresholds — read a rule frontmatter `params:` block, fall back to code defaults.

Editing the memory `.md` frontmatter is equivalent to changing thresholds; no hook restart needed.
...
params:
  chinese_ratio_threshold: 0.55   # default 0.7, lower for projects with many English loanwords
  min_length: 80                  # default 50
---
```

When a rule file has no `params:` block, callers get the default.

Per `code-pref-287` path decoupling; this module uses `path_config.get_memory_dir()`.
Per `wf-pref-292` adaptive threshold mechanism (user approves, never changed silently).
"""
import os
import re
import sys
...
if __name__ == '__main__':
    """Debug: list the current params of the 3 enforce rules."""
