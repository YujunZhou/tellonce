...
log "Tellonce install — version 1.6.0"
log "  HOME: ${HOME}"
...
log "Key paths:"
log "  - skill: ${SKILL_DIR}"
log "  - hooks: ${SKILL_DIR}/hooks (registered in-place; nothing is copied to the project)"
log "  - state: ${STATE_DIR}"
...
log "  - Run dashboard.sh to view the latest shadow-judge alerts (only populated in full mode)"
