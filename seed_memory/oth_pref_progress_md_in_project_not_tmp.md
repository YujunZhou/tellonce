...
description: Progress / status / report 类 MD 文件必须写在项目目录内, 不放 /tmp; 结果数据 JSON 可放 /tmp
type: preference
domain: other
scope: global
confidence: high
...
- `docs/` — 项目级 roadmap / 规划
- `notes/` — 你自己的工作笔记
- `experiments/<name>/` — 单次实验相关 report
- `reports/<date>/` — 阶段性结果分析
...
**Why:** 用户曾要求 "progress 的 md 不要写在 tmp 里, 就写在项目里". /tmp 是 ephemeral, 重启 / 清 cache 就丢; user 要 review 的文档应该跟项目一起有持久路径 + 能被 future session 找到.
